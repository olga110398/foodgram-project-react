...
from recipes.models import Ingredient, Tag, Recipe, IngredientAmount, Subscribe, Favorite
...
class RecipeListSerializer(serializers.ModelSerializer):
    ...
    ingredients = IngredientAmountSerializer(many=True, source='ingredient_amount')
...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not ingredients:
            raise serializers.ValidationError({'ingredients': 'Нужно выбрать ингердиенты!'})
        items = []
        for item in ingredients:
            items.append(item['id'])
        if len(items) != len(set(items)):
            raise serializers.ValidationError({'ingredients': 'Ингредиенты не уникальны!'})
        return data
...
class SubscribeListSerializer(serializers.ModelSerializer):
    ...
    def get_recipes(self, obj):
        ...
        return RecipeMiniFieldSerializer(recipes, many=True, context=self.context).data
...
class SubcribeSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        return SubscribeListSerializer(instance.following, context=self.context).data
...
class FavoriteSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        return RecipeMiniFieldSerializer(
            instance.recipe,
            context=self.context.get('request')).data
